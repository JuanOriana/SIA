...
if __name__ == '__main__':
    ...
    for run in range(int(runs)):
        ...
        for select_fun in ['rank_selection', 'roulette_selection']:
            ...
            for mutation_prob in [0.01, 1]:
                ...
                if run == 0: average_aptitudes[select_fun].update({mutation_prob: list()})
                if run == 0: max_aptitudes[select_fun].update({mutation_prob: list()})
                for i in range(len(chunked_generations)):
    ...
    ##
    #TODO: mutation_std usando el chunked max_gens (0.1,4)
    ...
    #
    # TODO:Limited variando el k ( ESTE TEST REVISARLO)
